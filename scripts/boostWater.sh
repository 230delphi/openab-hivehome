...
if [[ "$mode" == BOOST ]]; then
  ...
  thermostatModeOLD=`echo $infoNodes | jq .nodes[$HOTWATER_NODE].attributes.activeHeatCoolMode.reportedValue -r` #-r strips the quotes
  echo running new $RECEIVER_ID_WATER num
  thermostatMode=`echo $infoNodes | jq --arg RECEIVER_ID_WATER $RECEIVER_ID_WATER '.nodes[] | select(.id == $RECEIVER_ID_WATER).attributes.activeHeatCoolMode.reportedValue' -r`
  echo $thermostatModeOLD :: $thermostatMode
  ...
  if [ "BOOST" != "$thermostatMode" ]; then
	...
	prevMode=`echo $infoNodes | jq --arg RECEIVER_ID_WATER $RECEIVER_ID_WATER '.nodes[] | select(.id == $RECEIVER_ID_WATER).attributes.previousConfiguration.reportedValue.mode' -r`
    #prevTemp=`echo $infoNodes | jq .nodes[$HOTWATER_NODE].attributes.previousConfiguration.reportedValue.targetHeatTemperature`
	prevTemp=`echo $infoNodes | jq --arg RECEIVER_ID_WATER $RECEIVER_ID_WATER '.nodes[] | select(.id == $RECEIVER_ID_WATER).attributes.previousConfiguration.reportedValue.targetHeatTemperature' -r`
...
target=`echo $infoNodes | jq --arg RECEIVER_ID_WATER $RECEIVER_ID_WATER '.nodes[] | select(.id == $RECEIVER_ID_WATER).attributes.targetHeatTemperature.reportedValue' -r`
...
activeScheduleLock=`echo $infoNodes | jq --arg RECEIVER_ID_WATER $RECEIVER_ID_WATER '.nodes[] | select(.id == $RECEIVER_ID_WATER).attributes.activeScheduleLock.reportedValue' -r`
